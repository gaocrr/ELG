...
from models import TSP_Encoder, TSP_Decoder, local_policy, _get_encoding
...
class MLP_Local_policy(nn.Module):

    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params
        self.mlp = local_policy(self.model_params)
    ...
    def one_step_rollout(self, state, cur_dist, cur_theta, scale, eval_type):
        ...
        if state.current_node is None:
            ...
            action_scores = self.mlp(dist=cur_dist, theta=cur_theta, scale=scale, ninf_mask=state.ninf_mask)
            # shape: (batch, pomo, problem)
